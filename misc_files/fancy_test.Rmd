...
states.top10.death <- states.top10.all %>% 
  filter(death >= 10) %>%
  select(-since10) %>% 
  group_by(state) %>% 
  mutate(since10 = rank(date))
...
#### Prevalence 
```{r states top 10, fig.height= 7, fig.width = 10}
states.top10.all %>% 
...
states.top10.death %>% 
  ...
  geom_line(alpha = 0.7, size = 1.5, arrow = arrow(angle = 15, type = "closed", length = unit(0.1, "inches"))) + 
  stat_function(fun = function(x) 10*2^((x)/2), color= "grey", alpha = 0.5, linetype = "dashed",
  ...
  geom_label_repel(data = states.top10.death %>%
                     filter(date == max(date)) %>% 
                     arrange(desc(death)) %>% 
                     head(5),
                   aes(label = state), show.legend = FALSE,
                   alpha = 0.9, nudge_x = 0.25, nudge_y = 0.25, 
                   point.padding = 0.2, box.padding = 0.35) +
  coord_cartesian(ylim=c(0,max(states.top10.death$death, na.rm = TRUE))) +
  annotate("text", x =  10, y = 400, angle = 80, color = "grey",
           label = "Double every 2 days.", size = 3.5)
```
...
states.top10.all %>% 
  ggplot(aes(since10, death, group = state, color = state)) +
   stat_function(fun = function(x) log10(10*2^((x-10)/2)), color = "grey", alpha = 0.5, linetype = "dashed",
                arrow = arrow(angle = 15, type = "closed", length = unit(0.1, "inches")))+
  geom_line(alpha = 0.7, size = 1.5, arrow = arrow(angle = 15, type = "closed", length = unit(0.1, "inches"))) + 
  theme_minimal_hgrid() +
  scale_color_calc() + 
...
states.top10.all %>% 
  ...
  scale_y_log10() + 
  labs(y= "Logarithmic Reported Cases", x = "Days since reporting 10th case", color = "State",
       caption = "\nDashed grey line shows the path of a state if cases double every 2 days.") +
  ggtitle("Logarithmic cumulative reported cases by days\nsince the 10th reported case.") +
  geom_label_repel(data = states.top10.all %>%
                   ...
                   alpha = 0.9, nudge_x = 0.25, nudge_y = 0.25, 
                   point.padding = 0.2, box.padding = 0.35) + 
      annotate("text", x =  22, y = 15000, angle = 33, color = "grey",
...
current.state.new %>% 
  ggplot(aes(since10, new.cases, group = state, color = state)) +
  geom_line(data = states.new.top20, aes(since10, new.cases), color = "grey", alpha = 0.5, arrow = arrow(angle = 15, type = "closed", length = unit(0.08, "inches")))+ geom_label_repel(data = states.new.top20 %>%
                     filter(date == max(date)) %>% 
                     arrange(desc(since10)) %>% 
                     filter(state != the.state) %>% 
                     head(6),
...
current.state.new %>% 
  ggplot(aes(since10, new.deaths, group = state, color = state)) +
  geom_line(data = states.new.top20, aes(since10, new.deaths), color = "grey", alpha = 0.5, arrow = arrow(angle = 15, type = "closed", length = unit(0.08, "inches"))) + geom_label_repel(data = states.new.top20 %>%
                     filter(date == max(date)) %>% 
                     arrange(desc(since10)) %>% 
                     filter(state != the.state) %>% 
                     head(6),
